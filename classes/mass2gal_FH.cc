                                     ...
                                     bool extinct, bool AMcut, double SkyArea, bool GoldCut, bool ProbaCut)
{
  ...
  for(sa_size_t iz=0; iz<ngals_.SizeZ(); iz++)// Z direction (redshift or z-axis direction)
    for(sa_size_t iy=0; iy<ngals_.SizeY(); iy++) { // Y direction (transverse plane)
	  ...
	  double lambdamin=300.;  // on nanometre
	  double lambdamax=1000.;
	  
	  // importing the filter shapes
	  string filterfile_B("B_wfi_eso.txt");  // ../filters/g_SDSS.txt");
	  cout << "* Reading Filter from file " << filterfile_B << endl;
	  Filter filt_B(filterfile_B, lambdamin*1e-9, lambdamax*1e-9);
	  
	  // Importing a SED file for elipticals, spirals and star bursts
	  string sedfileEl("El_cww_fix2.txt"); // ../SEDs/El_cww_fix2.txt");
	  
	  cout<<"* Reading SED file for Ellipticals: "<<sedfileEl<<endl;
	  SED sedEl(sedfileEl,lambdamin*1e-9, lambdamax*1e-9);
	  ...
	  SEDzFilterProd integrand_B(sed,filt_B); // z=0
	  ...
	  double KrB = -2.5*log10(Fr/FB);
	  cout << "   ComputeBlim:  K-correction=2.5log(FB/Fr)= " << KrB << endl;
	  
    
	  appMag_i = KrB + 25. + 5.*log(rr) + mag;
