...
// of survey area
{
  ...
  for (int ic=0; ic<Nobsfiles; ic++) {
	...
	Rec2EuclidCoord(grec,x,y,z,redshift);
      ...
      // is replaced by the following line      AddToCell(x,y,z,selphi); 
      for (size_t igd=0; igd<vgrids_.size(); igd++) {
	if (vgrids_[igd].AddGal(x,y,z,selphi)) {
	  ngw_[igd] +=1/selphi; 
	  ng_[igd] ++;
	} else ngout_[igd] ++;
    ...
    for (size_t igd=0; igd<vgrids_.size(); igd++) {
      cout <<"    Number of galaxies inside grid n." << igd << " = "<< ng_[igd] <<endl;
      cout <<"    Number of 'galaxies' inside WEIGHTED grid n." << igd << " = "<< ngw_[igd] <<endl;
      cout <<"    .... (should be about equal to the number of galaxies in FULL catalog - not printed here)"<<endl;
      cout <<"    Number of galaxies outside grid n." << igd << " = "<< ngout_[igd] <<endl;
    }
  ...
  for (size_t igd=0; igd<vgrids_.size(); igd++) {
    if (ng_[igd]!=ngals_[igd].Sum())
      throw ParmError("ERROR! galaxy number discrepancy: ng_!=ngals_.Sum()");
    if (ngo_!=ngals_[igd].Sum())
      cout <<"    Number of galaxies outside grid = "<< ngout_[igd] <<endl;
  }
  ...
  for(int i=0; i<Nx_; i++) {
    ...
    for(int j=0; j<Ny_; j++) {
      for(int k=0; k<Nz_; k++)  {
        ...
        if (thetac<=SkyArea_)   {
	  // VERIF CALCULS INUTILES wieghts
...
void Cat2Grid::NormNArrays(){    
  ...
  if (vgrids_.size()>0) {
    ...
    for(size_t i=0; i<vgrids_.size(); i++) vgrids_[i] *= normw;
  }
  else {
...
void  Cat2Grid::WriteHeader(string IncatName) {
  ...
  fos_.WriteKey("NumberOfGrids",vgrids_.size()," number of grids");
