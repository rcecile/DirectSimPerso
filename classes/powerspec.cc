...
PowerSpec::PowerSpec(TArray<r_8> galdens, double Dx, double Dy, double Dz, double ratio_AngDiam)
  : drho_(galdens)
...
PowerSpec::PowerSpec(TArray<r_8> galdens, double Dx, double ratio_AngDiam)
: drho_(galdens)
// Reads in galaxy fluctuation grid and computes Fourier transform
...
void PowerSpec::CheckPZErr()
{
	...
	if(Err_>0) {
		...
		if (maxk_<Dkz_) {
			...
			//cout <<"    Changing maximum radial k allowed from "<< maxk_;
			// <<" to "<< Dkz_+1e-4 <<" or power spectrum will be zero"<<endl;
			//maxk_=Dkz_+1e-4;
...
// tol_corr: if divide Fourier coefficient by number greater than this power spectrum will blow up
 { 
   ...
   for(sa_size_t iz=0; iz<four_.SizeZ(); iz++) { // assumed RADIAL direction
	 ...
	 double kmod_z = kz;
	 double kmod_xy = sqrt((kx*kx+ky*ky));
	 ...
	 double pk_z   = za.real()*za.real()+za.imag()*za.imag(); // FAUX POUR LE MOMENT
	 double pk_xy = za.real()*za.real()+za.imag()*za.imag(); // FAUX POUR LE MOMENT
...
void PowerSpec::Write2PS(string fname, HProf& hp1, HProf& hp2, double VolCat,
                                                          double md, double mdf)
// writes the power spectrum to a text file
// the format is:
...
// Err is the standard deviation of the photo-z errors in comoving coordinates
 { 
	...
	cout << " d3k = "<< FourierVolEl <<", sum="<< sum;
	cout  <<", int_Wmodsq="<< int_Wmodsq <<endl;
...
// std Gsig.  Gaussian is in z-dimension direction
{
	...
	mydim[0]=Nx_=four_.SizeX(); // why is Nx in here? bug?
	mydim[1]=four_.SizeY(); 
	mydim[2]=four_.SizeZ();
