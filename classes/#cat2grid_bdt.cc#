                  ...
                  // Print: if true prints some extra info to screen whilst in constructor
{
  ...
  Ibdt_ = dt_.IndexNom("BDT");
  Ipodds_ = dt_.IndexNom("PODDS");
...
void Cat2Grid::SaveSelecFunc(string SFTextFile, string FullCat, string ObsCat, string ZFCol, string  ZSCol,  string ZOCol, bool MakeFullHist, double BDTcut, double PODDScut)
...
// Histo object
{
  ...
  if (MakeFullHist) {
    ...
    for (int ic=0; ic<Nfiles; ic++) {
      ...
      if (BDTcut != 99.)   cout <<" Read also BDT variable with a cut below "<< BDTcut <<endl;
      if (PODDScut != 99.) cout <<" Read also PODDS variable with a cut below "<< PODDScut <<endl;
...
// of survey area
{
  ...
  for (int ic=0; ic<Nobsfiles; ic++) {
    ...
    cout <<", TOTAL number of galaxies in loop = "<< ngall_ <<endl;
...
void Cat2Grid::Row2Record(DataTableRow& rowin, GalRecord& rec)
{
  ...
  rec.MB = rowin[Imag_];
  rec.BDT = rowin[Ibdt_];
  rec.PODDS = rowin[Ipodds_];
  // cout << "Row2Record "<< rec.zs << " " <<  rec.zo << " " << rec.BDT << " " <<  rec.PODDS << endl;
  // rec.zs = SPECTRO Z
