...
class GalRecord {
 public:
  ...
  /** Constructor */
  GalRecord() { alpha=delta=glong=glat=xcoo=ycoo=zcoo=0.; zs=zo=0.; type=1; u=g=r=i=z=y=24.; MB=-22; BDT=PODDS=99;}

  /** Print galaxy properties */
  void Print()
  { cout <<"Printing GalRecord: ";
    cout <<"alpha="<< alpha <<", delta="<< delta <<", x="<< xcoo <<", y="<< ycoo <<
      ", bdt="<< BDT <<", podds="<< PODDS <<", zs="<< zs <<", zo="<< zo;}
  ...
  double BDT;      /**< BDT variable                                        */
  double PODDS;    /**< ODDS variable                                       */
};
...
class Cat2Grid
{
 public:
  ...
  void SetGrid(double Phi=0, double zl=-1, double zh=-1,double R_XY=8,double R_Z=8);
  ...
  void SetGrid(int_8 Nx, int_8 Ny, int_8 Nz, double R_XY,double R_Z, double zref); 
  ...
  void SaveSelecFunc(string SFTextFile, string FullCat, string ObsCat, string ZFCol="zs", string  ZSCol="zs",  string ZOCol="zp", bool MakeFullHist=true, double BDTcut = -1., double PODDScut = -1.);
  ...
  TVector<r_8> ReturnGridSpec(){
    int row=5; TVector<r_8> gridspec(row); 
    gridspec(0)=Nx_; gridspec(1)=Ny_; 
    gridspec(2)=Nz_; gridspec(3)=cellsizeXY_; gridspec(4)=cellsizeZ_;
    return gridspec; };
 ...
 protected:
  ...
  double cellsizeXY_;   /**< pixel size of grid in Mpc                        */
  double cellsizeZ_;   /**< pixel size of grid in Mpc                        */
  ...
  sa_size_t ngzok_;   /**< number of gals with defined redshift (podds)     */ 
  ...
  sa_size_t Ibdt_;    /**< index of phi or x column in data table           */
  sa_size_t Ipodds_;  /**< index of theta or y column in data table         */
