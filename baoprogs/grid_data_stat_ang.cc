...
int main(int narg, char* arg[]) {
	...
	SInterp1D z2distMoi;                /**< redshift to distance look up table   */
	z2distMoi.DefinePoints(zrs,codist,minz,maxz,2*nz);	
	double ang05 = atan(150./z2distMoi(0.5))*180/3.14159;
	double ang10 = atan(150./z2distMoi(1.0))*180/3.14159;
	double ang15 = atan(150./z2distMoi(1.5))*180/3.14159;
	double angcmb = atan(150./z2distMoi(1000.))*180/3.14159;
	cout << "   z=0.5 "<< ang05 << " deg" << endl;
	cout << "   z=1.0 "<< ang10 << " deg"<< endl;
	cout << "   z=1.5 "<< ang15 << " deg"<< endl;
	cout << "   z=1000 "<< angcmb << " deg"<< endl;
	
	for(int iw0=0;iw0<3; iw0++) {
	  for(int iwa=0;iwa<3; iwa++) {
	    for(int iz=0;iz<2;iz++) {
	      wDE = -1.5 + 0.5*float(iw0);
	      wDA = -0.5 + 0.5*float(iwa);
		...
		codist.push_back(cod); 
	      ...
	      z2distMoi.DefinePoints(zrs,codist,minz,maxz,2*nz);
	      if (iz == 0) {
		double wang05 = atan(150./z2distMoi(0.5))*180/3.14159;
		double wang10 = atan(150./z2distMoi(1.0))*180/3.14159;
		double wang15 = atan(150./z2distMoi(1.5))*180/3.14159;
		double wangcmb = atan(150./z2distMoi(1000.))*180/3.14159;
		cout << "w0,wa = " << wDE << ", " <<  wDA  << "  z=0.5 "<< wang05   << " deg   -->   " << (ang05-wang05)/ang05*100.<< "%"<<  endl;
		cout << "w0,wa = " << wDE << ", " <<  wDA  <<"   z=1.0 "<< wang10   << " deg   -->   " << (ang10-wang10)/ang10*100.<< "%"<<  endl;
		cout << "w0,wa = " << wDE << ", " <<  wDA  <<"   z=1.5 "<< wang15   << " deg   -->   " << (ang15-wang15)/ang15*100.<< "%"<< endl;
		cout << "w0,wa = " << wDE << ", " <<  wDA  <<"   z=1000 "<< wangcmb << " deg   -->   " << (angcmb-wangcmb)/angcmb*100.<< "%"<< endl << endl;
	      }	    }
