...
void usage(void) {
	...
	cout << " -b : BDTcut     cut value on the BDT variable                  "<<endl;
	cout << " -p : PODDScut   cut value on the PODDS variable                "<<endl;
...
int main(int narg, char *arg[])	{
	...
	char c;
	while((c = getopt(narg,arg,"hdF:H:O:o:z:b:p:")) != -1) { 
	    switch (c) {
		    case 'F' :
			    FullCat = optarg;
			    break;
		    ...
		    case 'z' :
			    ...
			    break;
		    case 'b' :
			    sscanf(optarg,"%lf",&BDTcut);
			    break;
		    case 'p' :
			    sscanf(optarg,"%lf",&PODDScut);
			    break;
 	            case 'd' :
			    DoDebug=true;
			    break;
	...
	if (BDTcut != 99. && PODDScut != 99.) {
	  cout << "!!!!!!!!!!!!!!!!!!!!!!!! User must CHOOSE between BDT cut and PODDS cut !!!!!!!!!!!!!!!!!!!!!!!!"<< endl; 
	  exit(-1);
	}
...
//	   inp.clear(ios::failbit);
 	    cout <<" DONE" << endl;
	    if (MakeFullHist)
	      cat.SaveSelecFunc(SFFileName, FullCat, ObsCat, ZFCol, ZSCol, ZOCol,MakeFullHist, BDTcut, PODDScut);
	    else
	      cat.SaveSelecFunc(SFFileName, FullHist, ObsCat, ZFCol, ZSCol, ZOCol,MakeFullHist, BDTcut, PODDScut);
