...
restore,dir+'histo_zs_zp_lfZuccaAllFalse'+suff[0]+'.sav'
n = n_elements(z)
nz = dblarr(n)
n_mpc = nz
...
for i=0,n_elements(suff)-1 do begin
   ...
   restore,dir+'histo_zs_zp_lfZuccaAllFalse'+suff[i]+'.sav'
   ...
   sn_zp = string(n/tot*100.,format=myformat)
   print,'fct de zP ',sn_zp
...
print,'!!!!!!!!!!! TABLE 1 !!!!!!!!!!!'
nsuff = ['z_S','z_G','z_p','z_{BDT9}','z_{BDT8}']
...
print,tit
for i=0,n_elements(nsuff)-1 do begin

   if(strcmp(nsuff[i],'z_S') eq 1) then name=suff[i+1] else name=suff[i-1]
   restore,dir+'histo_zs_zp_lfZuccaAllFalse'+name+'.sav'

   for iz=0,n_z-1 do begin
      ok = where (z ge zmin[iz] and z lt zmax[iz],nok)
      if(strcmp(nsuff[i],'z_S') eq 1) then n[iz] = total(all_hist[ok,*]) else n[iz] = total(all_hist[*,ok])
   endfor
   res = string(nsuff[i],n/surf,format=myformat)
   print,res
endfor
