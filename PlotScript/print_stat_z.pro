...
myformat = '(A10, F7.3,F7.3, F7.3, F7.3, F17.3)'

tit= '['+strtrim(zmin,2)+'-'+strtrim(zmax,2)+'['
print,tit
res=dblarr(10,n_z)
for icase=0,9 do begin
   restore,dir+'histo_zs_statZp_'+strtrim(icase,2)+suff[0]+'.sav'
   for iz=0,n_z-1 do begin
      ok = where (z ge zmin[iz] and z lt zmax[iz])
      n[iz] = total(all_hist[*,ok])
      res[icase,iz] = n[iz]/surf
   endfor
endfor

for iz=0,n_z-1 do print,mean(res[*,iz]),stddev(res[*,iz])
