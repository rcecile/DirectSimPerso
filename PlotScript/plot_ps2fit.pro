...
if (option eq 0) then begin
   suff=['','_err0.03','_errPBDT8']
   lerr=['spectroZ','Gaussian 0.03','photoZ BDT 80%']
   lpsym=[-1,-5,-4]
   lcol  = [95,210,120]
...
if (option eq 3) then begin
   ...
   lerr=['spectroZ','photoZ BDT 90%','photoZ BDT 80%']
   lpsym=[-1,-5,-3]
   lcol  = [95,135,120]
   wtit='_S98'
endif
...
for ir=0,n_elements(suff)-1 do begin
  ...
  ; spectre observe corrige du damping de l'erreur 
   t  =  dir + 'PS_cube_2fitErr'+nx[iz]+'_z'+namez[iz]+mysuff+'_wngal.txt'
;   if (ir eq 2 and iz eq 2) then t  =  dir + 'PS_cube_2fitErr'+nx[iz]+'_z'+namez[iz]+'_k0.06'+mysuff+'_wngal.txt'
   ...
   legend,'z = '+namez[iz],box=1,/fill,/right,/bottom,charsize=2

   ;fit
;   ffit = dir + 'fit_cube_Err'+skmax[iz]+nx[iz]+'_z'+namez[iz]+mysuff+'_result.txt'
;   ffit = dir + 'fit_cube_Err'+nx[iz]+'_z'+namez[iz]+mysuff+'_result.txt'
;   print,ffit
...
;   chi2fit = dir + 'fit_cube_Err'+skmax[iz]+nx[iz]+'_z'+namez[iz]+mysuff+'_chisq.txt'
...
;legend,lerr+kpt,col=lcol,li=0,box=1,/fill,/right,/top,charsize=1.5
;print,"========================================================= "
;print,serr
;print,"========================================================= "
;print,kerr
