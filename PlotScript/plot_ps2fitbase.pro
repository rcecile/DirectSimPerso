...
; current plotting device.
   mydevice = !D.NAME
   SET_PLOT, 'PS'
   DEVICE, FILENAME='/sps/lsst/users/rcecile/Fig/plot_ps2fitbase.eps', /PORTRAIT,/COLOR,XSIZE=8.8,YSIZE=8.8,FONT_SIZE=4
endif
...
myytit=['oscillation PS [(Mpc)^-3]','oscillation PS [(Mpc)^-3]','oscillation PS [(Mpc)^-3]']
...
for ierr=0,2 do begin
   ...
   plot,xs,ps,/xs,/ys,xra=[0.021,0.149],yra=[0.91,1.09],ytit=myytit[ierr],xtit=myxtit[ierr],xma=[myxmargin1[ierr],myxmargin2[ierr]],yma=[myymargin1[ierr],myymargin2[ierr]],/nodata
   oplot,[kfid,kfid],[0,2],th=2
   oplot,[0,.2],[1,1],th=2
   oplot,xs[ok],ps[ok],col=lcol[ierr],psym=lpsym[ierr],sym=1.5
...
;  oplot,2.*!pi/[pfitw.(0)[0],pfitw.(0)[0]],[0,2],li=1,th=1
  legend,lerr[ierr],charsize=1.5,li=0,psym=lpsym[ierr],col=lcol[ierr],/bottom,/right,/box
endfor
endfor
