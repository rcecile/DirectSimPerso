...
sigma=[0.030515025d, 0.028397875d ,0.037388461d,$

;#erreur mediane photoZ BDT90
0.025927096d, 0.027347027d, 0.032890236d, $

;#erreur mediane photoZ BDT80
0.023046306d, 0.026009080d, 0.030297966d]
...
; current plotting device.
   mydevice = !D.NAME
   SET_PLOT, 'PS'
   DEVICE, FILENAME='/sps/lsst/dev/rcecile/Fig/plot_PS_noCata_noBias.eps',/PORTRAIT,/COLOR,XSIZE=8.8,YSIZE=6.6,FONT_SIZE=4
endif 

for iz=0,n_elements(D)-1 do begin

   if (iz eq 0) then plot,[0.04,0.2],[0.,20],/nodata,/xs,/ys,xra=[0.01,0.15],xticknam=replicate(" ",10),ytit='difference [%]',xma=[8,.5],yma=[0,.5],yra=[0,11] $
   else plot,[0.04,0.2],[0.,20],/nodata,/xs,/ys,xra=[0.01,0.15],yra=[0,11],xtit='Wavenumber [Mpc^-1]',ytit='normalized PS',xma=[8,.5],yma=[3,0]

   for ir =0,n_elements(errlist)-1 do begin

      name=dir+"PS_noCata_noBias_"+snx[iz]+"_z"+D[iz]+errlist[ir]+"_wngal.txt"
      result = FILE_TEST(name)
      if (result eq 0) then continue
      
      p1 = read_ascii(name, template =  TEMP_2FIT)
      
      name=dir+"PS_cube_"+snx[iz]+"_z"+D[iz]+"_err"+ strtrim(sigma[iz+3*ir],2)+ "_wngal.txt"
