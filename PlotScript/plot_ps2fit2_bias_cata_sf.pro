PRO plot_ps2fit2_bias_cata_sf,saveplot
...
mysuff=['_errPBDT8','_errP','_errPBDT8']
nerr=['photoZ BDT 80%','photoZ','photoZ BDT 80%']


suff_eff=['_noBias','_noCata','_SFspec']
leff=['no bias','no outliers','simple selection function']
lcol  = [50,230,200]
lpsym=[1,6, 4]
lpsym=[4,4, 4]
...
kmax = intarr(n_elements(namez))
kmax[*] = [16,16,10]
skmax = strarr(n_elements(namez))
for i=0,n_elements(namez)-1 do $
...
; current plotting device.
   mydevice = !D.NAME
   SET_PLOT, 'PS'
   DEVICE, FILENAME='/sps/lsst/dev/rcecile/Fig/plot_ps2fit_cas.eps', /PORTRAIT,/COLOR,XSIZE=17.6,YSIZE=12,FONT_SIZE=4
endif
...
for ierr=0,2 do begin
   ...
   t  =  dir + 'PS_2fit'+suff_eff[ierr]+nx[iz]+'_z'+namez[iz]+mysuff[ierr]+'_wngal.txt'
   print,t
   pcorr = read_ascii(t, template =  TEMP_2FIT)     
   xs = pcorr.(0)
   okk = where(xs ge 0.02 and xs le kmax[iz]/100.)
   ...
   plot,xs[okk],ps[okk],/xs,/ys,xra=[0.021,0.199],yra=[0.91,1.09],ytit=myytit[ierr],xtit=myxtit[iz],$
        xma=[myxmargin1[ierr],myxmargin2[ierr]],yma=[myymargin1[iz],myymargin2[iz]],/nodata 
  ...
  ffitw = dir + 'fit'+suff_eff[ierr]+skmax[iz]+nx[iz]+'_z'+namez[iz]+mysuff[ierr]+'_result.txt'
  print,ffitw
...
legend,leff+', '+nerr,charsize=1.5,li=1,psym=lpsym,col=lcol,/bottom,/right,/box
