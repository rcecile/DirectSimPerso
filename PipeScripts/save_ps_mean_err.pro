...
err_tot=['_err0.023', '_err0.021', '_err0.029', '_err0.0198', '_err0.020', '_err0.025', '_err0.0173', '_err0.0191', '_err0.0228']
; erreur photoZ
...
for iz = 0,nz-1 do begin
   ...
   for ir = 0, nerr-1 do begin
      suff = nameerr[ir]
      sufferr = err_tot[iz+3*ir]
      print,namez[iz],' ',suff,' ',sufferr
      for igd =0,4 do begin
                                ; spectre observ??
         fobs  = dir + 'PS'+psuff+nx[iz]+'_z'+namez[iz]+suff+'_G'+strtrim(igd,2)+'_wngal.txt' 
 ;        fobs  = dir + 'PS'+refsuff+nx[iz]+'_z'+namez[iz]+sufferr+'_G'+strtrim(igd,2)+'_wngal.txt' 
         print,fobs
                                ;    print,fobs
      ...
      ; < spectre simul?? no osc, with error >
         ...
         for j=0,nsim-1 do begin
            fnobao  = dirn + 'PS'+refsuff+'_'+strtrim(j,2)+nx[iz]+'_z'+namez[iz]+sufferr+'_G'+strtrim(igd,2)+'_wngal.txt' 
      ;      print,fnobao
      ...
      fname = dir + 'PS_errGauss_2fit'+nx[iz]+'_z'+namez[iz]+suff+'_wngal.txt' 
                                ;    print,fname
